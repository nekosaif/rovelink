...
class XBeeBenchmark:
            ...
            try:
                ...
                if not self.serial_conn or not self.serial_conn.is_open:
                    ...
                    if self.connect():
                        ...
                        print("Failed to reconnect. Stopping receive thread.")
                        break
                ...
                if time.time() - last_activity > connection_check_interval:
                    if self.mode == 'slave':
                        print(f"[{datetime.now().strftime('%H:%M:%S')}] Slave node active - waiting for master...")
                    last_activity = time.time()
                    
            except serial.SerialException as e:
                print(f"Serial error in receive thread: {e}")
    ...
    def handle_received_message(self, message):
        ...
        if msg_type == self.MSG_PING:
            ...
            if self.mode == 'slave':
                print(f"DATA received ({message['data_size']} bytes)")
            
        elif msg_type == self.MSG_SYNC:
            print(f"Sync received from node {message['node_id']}")
            if self.mode == 'slave':
                print("Ready to respond to master requests...")
    ...
    def run_comprehensive_test(self):
        ...
        if self.mode == 'master':
            ...
            print("Slave mode active - responding to master requests...")
            print("Press Ctrl+C to stop")
            
            # In slave mode, run continuously and respond to master
            try:
                while self.running:
    ...
    def start(self):
        ...
        try:
            ...
            # Print final results only for master mode
            if self.mode == 'master':
                print("\n" + "="*50)
                print("FINAL RESULTS")
                print("="*50)
                ...
                print("\nMaster tests completed. Press Ctrl+C to exit.")
                # Keep master running to receive slave results
                while self.running:
                    time.sleep(1)
